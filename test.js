...
test('katalog maya strip; void/endirim payments blok', function () {
  ...
  assert.ok(html.indexOf('orders-ui.js?v=50') >= 0);
...
test('çek paneli: sec-actions main-dən əvvəl; premium kart', function () {
  ...
  assert.ok(html.indexOf('orders.css?v=43') >= 0);
...
test('PWA ofisiant: manifest mode=waiter; waiter-mode hook', function () {
  ...
  assert.ok(html.indexOf('id="floor-fit"') >= 0);
  assert.ok(html.indexOf('id="floor-zoom-in"') >= 0);
  assert.ok(html.indexOf('Zal planı') >= 0);
  const css = fs.readFileSync(path.join(__dirname, 'public', 'orders.css'), 'utf8');
  ...
  assert.ok(css.indexOf('.floor-view-controls') >= 0);
  assert.ok(html.indexOf('id="waiter-basket-badge"') >= 0);
  const ui = fs.readFileSync(path.join(__dirname, 'public', 'orders-ui.js'), 'utf8');
  assert.ok(ui.indexOf('function floorFitToScreen') >= 0);
  assert.ok(ui.indexOf('scheduleInitialFloorFit') >= 0);
  assert.ok(ui.indexOf('translate3d(') >= 0);
  assert.ok(ui.indexOf('naturalMapSize') >= 0);
  assert.ok(ui.indexOf('floorFitToScreen') >= 0);
  assert.ok(ui.indexOf('bindFloorPanZoom') >= 0);
  assert.ok(ui.indexOf('arpos-floor-view-v2') >= 0);
...
test('1.2.74 masa sahibliyi: ofisiant takeover yox; kassir/admin var; API 403', function () {
  ...
  assert.strictEqual(require('./package.json').version, '1.2.84');
});
