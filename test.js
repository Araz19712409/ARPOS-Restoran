...
test('katalog maya strip; void/endirim payments blok', function () {
  ...
  assert.ok(html.indexOf('orders-ui.js?v=45') >= 0);
...
test('çek paneli: sec-actions main-dən əvvəl; premium kart', function () {
  ...
  assert.ok(html.indexOf('orders.css?v=38') >= 0);
...
test('PWA ofisiant: manifest mode=waiter; waiter-mode hook', function () {
  ...
  assert.ok(html.indexOf('id="floor-map-zoom"') >= 0);
  assert.ok(html.indexOf('id="floor-orient-h"') >= 0);
  const css = fs.readFileSync(path.join(__dirname, 'public', 'orders.css'), 'utf8');
  ...
  assert.ok(ui.indexOf('packRoomsVertical') >= 0);
  assert.ok(ui.indexOf('arpos-floor-map-zoom') >= 0);
  assert.ok(ui.indexOf('arpos-floor-map-orient') >= 0);
...
test('1.2.74 masa sahibliyi: ofisiant takeover yox; kassir/admin var; API 403', function () {
  ...
  assert.strictEqual(require('./package.json').version, '1.2.78');
});
