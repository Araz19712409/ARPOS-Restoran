...
test('növbə avto yalnız pay; false-da yox; accept/fire yox', function () {
  ...
  assert.ok(src.indexOf('function requireReserveAdmin') >= 0);
  assert.ok(src.indexOf('Sizin buna icazəniz yoxdur.') >= 0);
  const reserveFn = sliceBetween("app.post('/api/reservations'", "app.post('/api/reservations/:id/prepay'");
  assert.ok(reserveFn.indexOf('requireReserveAdmin') < 0);
  assert.ok(reserveFn.indexOf('waiterId: staff.user.id') >= 0);
...
test('pulsuz: Endirim yox → accept 403; kataloq 0 OK; pending gizlə; comp', function () {
  ...
  assert.ok(pending.indexOf('Pulsuz') >= 0);
  assert.ok(pending.indexOf('buildCheckMenu') >= 0 || ui.indexOf('buildCheckMenu') >= 0);
...
test('çek paneli: sec-actions main-dən əvvəl; premium kart', function () {
  ...
  assert.ok(sumAt >= 0 && hintAt > sumAt && msgAt > hintAt && secAt > msgAt && mainAt > secAt);
  assert.ok(moreAt > secAt && moreAt < quickAt && quickAt < mainAt);
  ...
  const wizHide = css.slice(
    css.indexOf('.order-wizard[data-zone="floor"] .check-head'),
    css.indexOf('.order-wizard.order-page[data-zone="groups"] .check-more')
  );
  assert.ok(wizHide.indexOf('[data-zone="floor"] .check-more') >= 0);
  assert.ok(wizHide.indexOf('[data-zone="groups"] .check-more') < 0);
  assert.ok(css.indexOf('.order-wizard.order-page[data-zone="groups"] .check-more') >= 0);
  assert.ok(css.indexOf('.order-wizard.order-page[data-zone="menu"] .check-more') >= 0);
});

test('sifariş məhsul kartı: compact ad+qiymət', function () {
  const css = fs.readFileSync(path.join(__dirname, 'public', 'orders.css'), 'utf8');
  assert.ok(css.indexOf('--icon-size') >= 0);
  assert.ok(css.indexOf('--card-min-h') >= 0);
  assert.ok(css.indexOf('-webkit-line-clamp: 2') >= 0);
  assert.ok(css.indexOf('aspect-ratio: 4 / 3') < 0);
  assert.ok(css.indexOf('.order-card .price') >= 0);
  assert.ok(css.indexOf('color: #e2b65a') >= 0);
  const cardBlock = css.slice(css.indexOf('.order-card {'), css.indexOf('.order-card .photo {'));
  assert.ok(cardBlock.indexOf('display: flex') >= 0);
  const ui = fs.readFileSync(path.join(__dirname, 'public', 'orders-ui.js'), 'utf8');
...
test('sifariş 1.2.69: sticky fav, mənim masalarım, scroll oxları', function () {
  ...
  assert.ok(html.indexOf('floor-toolbar') >= 0);
...
test('sağ çek premium: ad × miqdar + məbləğ', function () {
  const ui = fs.readFileSync(path.join(__dirname, 'public', 'orders-ui.js'), 'utf8');
  assert.ok(ui.indexOf('function lineSumText') >= 0);
  assert.ok(ui.indexOf('function buildCheckMenu') >= 0);
  assert.ok(ui.indexOf('check-pending-head') >= 0);
  assert.ok(ui.indexOf('line-amt') >= 0);
  assert.ok(ui.indexOf('check-ellipsis') >= 0);
  assert.ok(ui.indexOf('stepper') >= 0);
  const css = fs.readFileSync(path.join(__dirname, 'public', 'orders.css'), 'utf8');
  assert.ok(css.indexOf('minmax(0, 1fr) auto auto') >= 0);
  assert.ok(css.indexOf('.check-row.pending') >= 0);
...
test('məhsul qrupa keçir: modal + PUT groupId', function () {
  const html = fs.readFileSync(path.join(__dirname, 'public', 'products.html'), 'utf8');
  assert.ok(html.indexOf('id="move-group-modal"') >= 0);
  assert.ok(html.indexOf('Qrupa keçir') >= 0);
  assert.ok(html.indexOf('products.js?v=7') >= 0);
  assert.ok(html.indexOf('products.css?v=2') >= 0);
  const js = fs.readFileSync(path.join(__dirname, 'public', 'products.js'), 'utf8');
  assert.ok(js.indexOf('openMoveGroupModal') >= 0);
  assert.ok(js.indexOf("JSON.stringify({ groupId: nextId })") >= 0);
  assert.ok(js.indexOf('Qrupa keçir') >= 0);
  const serverSrc = fs.readFileSync(path.join(__dirname, 'server.js'), 'utf8');
  assert.ok(serverSrc.indexOf('qrup dəyişdi') >= 0);
  assert.ok(serverSrc.indexOf('groupMove') >= 0);
});
...
test('PWA ofisiant: manifest mode=waiter; waiter-mode hook', function () {
  ...
  assert.ok(html.indexOf('id="floor-map-controls"') >= 0);
  assert.ok(html.indexOf('id="floor-map-scale-x"') >= 0);
  assert.ok(html.indexOf('id="floor-map-scale-y"') >= 0);
  assert.ok(html.indexOf('id="floor-drag-e"') >= 0);
  assert.ok(html.indexOf('id="floor-drag-s"') >= 0);
  const css = fs.readFileSync(path.join(__dirname, 'public', 'orders.css'), 'utf8');
  assert.ok(css.indexOf('.order-wizard') >= 0);
  assert.ok(css.indexOf('.order-wizard .order-zones') >= 0);
  assert.ok(css.indexOf('data-zone="groups"') >= 0);
  assert.ok(css.indexOf('.tile-waiter') >= 0);
  assert.ok(css.indexOf('.waiter-basket-badge') >= 0);
  assert.ok(css.indexOf('min-width: 901px') >= 0);
  assert.ok(css.indexOf('.floor-map') >= 0);
  assert.ok(css.indexOf('.floor-toolbar-row') >= 0);
  assert.ok(css.indexOf('.floor-map-controls') >= 0);
  assert.ok(html.indexOf('id="waiter-basket-badge"') >= 0);
  const ui = fs.readFileSync(path.join(__dirname, 'public', 'orders-ui.js'), 'utf8');
  assert.ok(ui.indexOf('function fitFloorMap') >= 0);
  assert.ok(ui.indexOf('naturalMapSize') >= 0);
  assert.ok(ui.indexOf('naturalMapSize(boxes)') >= 0);
  assert.ok(ui.indexOf('arpos-floor-map-scale-x') >= 0);
  assert.ok(ui.indexOf('arpos-floor-map-scale-y') >= 0);
  assert.ok(ui.indexOf('function mountFloorMapDrag') >= 0);
  assert.ok(css.indexOf('.floor-map-handle') >= 0);
  assert.ok(ui.indexOf('function isWaiterMode') >= 0);
  assert.ok(ui.indexOf('function isOrderWizard') >= 0);
  assert.ok(ui.indexOf('function useFloorMap') >= 0);
  ...
  assert.ok(ui.indexOf('function syncWaiterBasketBadge') >= 0);
  assert.ok(ui.indexOf('Səbət boş') >= 0);
  assert.ok(ui.indexOf('floor-map-canvas') >= 0);
  ...
  assert.ok(zones.indexOf('order-wizard') >= 0);
...
test('ilkin doldurma bootstrap: create/update/station/qty', function () {
  ...
  assert.ok(html.indexOf('Cədvəli təmizlə') >= 0);
  assert.ok(html.indexOf('boot-clear-after') >= 0);
  assert.ok(html.indexOf('boot-import') >= 0);
  assert.ok(html.indexOf('Excel-dən yüklə') >= 0);
  assert.ok(html.indexOf('Şablon endir') >= 0);
  assert.ok(html.indexOf('boot-import-wipe') >= 0);
  assert.ok(html.indexOf('vendor/xlsx.full.min.js') >= 0);
  assert.ok(html.indexOf('bootstrap.css?v=5') >= 0);
  assert.ok(html.indexOf('bootstrap-ui.js?v=4') >= 0);
  const bootUi = fs.readFileSync(path.join(__dirname, 'public', 'bootstrap-ui.js'), 'utf8');
  assert.ok(bootUi.indexOf('localGroups') >= 0);
  assert.ok(bootUi.indexOf('Yarat: ') >= 0);
  assert.ok(bootUi.indexOf('createLocal') >= 0);
  assert.ok(bootUi.indexOf('rememberLocal') < 0);
  assert.ok(bootUi.indexOf('validateForCommit') >= 0);
  assert.ok(bootUi.indexOf('arpos-bootstrap-draft') >= 0);
  assert.ok(bootUi.indexOf('yaranacaq — davam?') >= 0);
  assert.ok(bootUi.indexOf('IMPORT_MAX') >= 0);
  assert.ok(bootUi.indexOf('parseCsvText') >= 0);
  assert.ok(bootUi.indexOf('parseXlsxBuffer') >= 0);
  assert.ok(bootUi.indexOf('downloadTemplate') >= 0);
  assert.ok(bootUi.indexOf('HEADER_ALIASES') >= 0);
  assert.ok(bootUi.indexOf('silent: true') >= 0);
  assert.ok(fs.existsSync(path.join(__dirname, 'public', 'vendor', 'xlsx.full.min.js')));
  const bootCss = fs.readFileSync(path.join(__dirname, 'public', 'bootstrap.css'), 'utf8');
  assert.ok(bootCss.indexOf('color: #1f1a14') >= 0);
  assert.ok(bootCss.indexOf('!important') >= 0);
  assert.ok(bootCss.indexOf('boot-err') >= 0);
  assert.ok(bootCss.indexOf('boot-plus') >= 0);
  assert.ok(bootCss.indexOf('#boot-import') >= 0);
  const serverSrc = fs.readFileSync(path.join(__dirname, 'server.js'), 'utf8');
  ...
  assert.ok(settingsHtml.indexOf('/bootstrap.html') >= 0);
  assert.ok(settingsHtml.indexOf('İlkin doldurma') >= 0);
